...
这份文档记录如何维护、构建、验证和发布 `OLE Packager`。
...
- `Insert OLE Package`：选择包含 `plot.png` 的图目录并插入 OLE 图包。
- `Insert Image + Files`：打开确认窗口；用户选择显示图片后，插件创建临时包内容文件夹，用户点击 `Open Folder...` 后用 Windows Explorer 管理内容，最后点击 `Insert` 提交。
- `Manage Image/OLE Files`：选择文档中已有图片或 OLE Packager 对象后打开确认窗口；普通图片会使用空的包内容文件夹，已有 OLE 对象会先把现有 zip 解压到包内容文件夹；用户在 Windows Explorer 中增删改文件后提交，两种模式都会保持原来的显示外观、大小和位置。
- `Usage`：打开带垂直滚动条的终端用户使用说明窗口。

Ribbon 图标：

```text
assets\insert-ole-package-icon.png
...
```powershell
powershell -ExecutionPolicy Bypass -File OLEPackager\tools\generate-ribbon-icons.ps1
```

Inno Setup 安装器脚本：

```text
installer\inno\OLEPackager.iss
```

UTF-8 zip helper：

```text
tools\OLEPackagerZipTool.cs
dist\OLEPackagerZipTool.exe
```
...
```powershell
& "$env:WINDIR\Microsoft.NET\Framework64\v4.0.30319\csc.exe" /nologo /target:exe /out:OLEPackager\dist\OLEPackagerZipTool.exe /reference:System.IO.Compression.dll /reference:System.IO.Compression.FileSystem.dll OLEPackager\tools\OLEPackagerZipTool.cs
```

输出：

```text
OLEPackager\dist\OLEPackagerZipTool.exe
```
...
```powershell
powershell -ExecutionPolicy Bypass -File OLEPackager\tools\build-word-addin.ps1
```

这个脚本会启动可见 Word，并使用当前验证过的稳定保存顺序：

1. 先保存一个 plain `.dotm` 到 `%LOCALAPPDATA%\Temp\OLEPackagerBuild\PlainSaveTest.dotm`，用于预热 Word 的模板保存路径。
2. 再创建第二个文档，导入 VBA 模块和 UserForm，保存为 `%LOCALAPPDATA%\Temp\OLEPackagerBuild\VbaSaveTest.dotm`。
3. 关闭 Word。
4. 把 `VbaSaveTest.dotm` 复制为 `dist\OLEPackager.dotm`。
...
```text
OLEPackager\dist\OLEPackager.dotm
```
...
customUI/images/insert-ole-package-icon.png
...
如果只想生成 `dist\OLEPackager.dotm`，不要加参数。  
如果要同时安装到本机 Word STARTUP 目录，使用：

```powershell
powershell -ExecutionPolicy Bypass -File OLEPackager\tools\build-word-addin.ps1 -Install -CloseWord
```

参数说明：

- `-Install`：构建后复制 `OLEPackager.dotm` 和 `OLEPackagerZipTool.exe` 到 `%APPDATA%\Microsoft\Word\STARTUP`。
...
```powershell
& "$env:LOCALAPPDATA\Programs\Inno Setup 6\ISCC.exe" "OLEPackager\installer\inno\OLEPackager.iss"
```

Inno 脚本会把当前 `dist` 中的两个文件打进安装包：

```text
dist\OLEPackager.dotm
dist\OLEPackagerZipTool.exe
```
...
```powershell
$p = Start-Process -FilePath (Resolve-Path "OLEPackager\release\OLEPackagerSetup.exe") -ArgumentList "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART" -Wait -PassThru
"InstallExitCode=$($p.ExitCode)"
Get-Item (Join-Path $env:APPDATA "Microsoft\Word\STARTUP\OLEPackager.dotm")
Get-Item (Join-Path $env:APPDATA "Microsoft\Word\STARTUP\OLEPackagerZipTool.exe")
```
...
```powershell
powershell -ExecutionPolicy Bypass -File OLEPackager\tools\run-word-script-with-timeout.ps1 -ScriptPath OLEPackager\tools\build-word-addin.ps1 -TimeoutSeconds 30
```
...
```powershell
powershell -ExecutionPolicy Bypass -File OLEPackager\test\run-word-smoke-test-with-temp-vbom.ps1
```
...
```powershell
powershell -ExecutionPolicy Bypass -File OLEPackager\test\run-word-smoke-test-with-temp-vbom.ps1 -TimeoutSeconds 30
```
...
当前主要依赖随加载项安装的 `OLEPackagerZipTool.exe`。这个 helper 由 `tools\OLEPackagerZipTool.cs` 编译而来，使用 .NET `System.IO.Compression.ZipArchive`，并显式使用 `System.Text.Encoding.UTF8` 读写 entry names。这样新生成或重建的 zip 会使用标准 UTF-8 文件名，尽量避免中文文件/文件夹名在其他现代压缩包查看器里乱码。

VBA 会按以下顺序寻找 helper：

1. 环境变量 `OLE_RAW_DATA_ZIP_TOOL` 指定的路径，主要用于测试。
2. Word STARTUP 目录：`Application.StartupPath\OLEPackagerZipTool.exe`。
3. `.dotm` 所在目录：`ThisDocument.Path\OLEPackagerZipTool.exe`。

发布前需要先把 helper 编译到 `dist\OLEPackagerZipTool.exe`；Inno 安装脚本会把 `OLEPackager.dotm` 和 `OLEPackagerZipTool.exe` 一起安装到 Word STARTUP 目录。

`Insert OLE Package` 的文件夹模式会排除顶层生成图文件。归档脚本会递归枚举源文件夹中的文件和空文件夹，把相对路径写成 zip entry name：
...
```text
%LOCALAPPDATA%\Temp\OLEPackager_Work_<baseName>_<timestamp>\
```
...
不要只发送 `.dotm`；没有 `OLEPackagerZipTool.exe` 时，UTF-8 zip 打包/列出/解压功能不可用。
...
2. 双击安装器。
3. 重新打开 Word。
4. 在 `OLE Packager` 选项卡点击 `Insert OLE Package`、`Insert Image + Files`，或先选中已有图片/OLE 对象再点击 `Manage Image/OLE Files`。
...
```text
Installed apps -> OLE Packager -> Uninstall
```
...
- 不建议把自动构建脚本交给终端用户。终端用户只使用 `OLEPackagerSetup.exe`。
